...
public class UserManagementService {
	...
	public PublicUserDetailsResponse getUser(
		UserLoginDetails userLoginDetails
	) {
		PublicUserDetailsResponse publicUserDetailsResponse = new PublicUserDetailsResponse();
		publicUserDetailsResponse.setPublicUserQueryResponseStatus(
			UserEnums.LOGIN_FAILED
		);
		...
		CompletableFuture<PublicUserDetailsResponse> getUserCompletableFuture = CompletableFuture.supplyAsync(() -> {
				try (
					...
					if (!rs.next()) {
						publicUserDetailsResponseFinal.setPublicUserQueryResponseStatus(
							UserEnums.NO_SUCH_USER
						);
					...
					if (
						...
						publicUserDetailsResponseFinal.setPublicUserQueryResponseStatus(
							UserEnums.INCORRECT_PASSWORD
						);
						return publicUserDetailsResponseFinal;
					}

					String validatedAuthKey = validateAndUpdateAuthKey(
					...
					userToReturn.setRegistrationDate(
					...
					if (avatarAsBlob != null) {
						byte[] avatarBytes = avatarAsBlob.getBytes(
							1,
							(int) avatarAsBlob.length()
						);
						String avatarAsBase64String = Base64
							.getEncoder()
							.encodeToString(avatarBytes);
						userToReturn.setUserAvatar(avatarAsBase64String);
					}

					publicUserDetailsResponseFinal.setPublicUserQueryResponseStatus(
						UserEnums.LOGIN_SUCCESSFUL
					);
					publicUserDetailsResponseFinal.setPublicUserDetails(
						userToReturn
					);
	...
	public UserEnums deleteUser(
		UserAuthKey userAuthKey,
		StoreManagementService storeManagementService
	) {
		String userOwnedStoreUUID = getUserOwnedStoreUUID(
			userAuthKey.getAuthKey()
		);
		...
		CompletableFuture<UserEnums> deleteUserFromDatabaseCompleyCompletableFuture = CompletableFuture.supplyAsync(() -> {
				try (
					...
					return (userDeleted > 0)
						? UserEnums.USER_DELETED
						: UserEnums.USER_DELETION_FAILED;
				} catch (SQLException e) {
					throw new RuntimeException(e);
				}
			}
	...
	// Update the user. I hate this method, it's ugly as sin but it works fine, it's just long and not very complex.
	public UserEnums updateUser(UserUpdate userUpdate) {
		...
		CompletableFuture<UserEnums> updateUsCompletableFuture = CompletableFuture.supplyAsync(() -> {
				try (
					Connection conn = DatabaseVerification.getConnection();
					PreparedStatement statement = conn.prepareStatement(
						"UPDATE Users SET userName = IF(?, ?, userName), firstName = IF(?, ?, firstName), lastName = IF(?, ?, lastName), email = IF(?, ?, email), hash = IF(?, ?, hash), salt = IF(?, ?, salt), avatar = IF(?, ?, avatar) WHERE authKey = ?"
					)
				) {
					statement.setBoolean(
						1,
						!userUpdate.getUserName().isEmpty()
					);
					statement.setString(2, userUpdate.getUserName());

					statement.setBoolean(
						3,
						!userUpdate.getFirstName().isEmpty()
					);
					statement.setString(4, userUpdate.getFirstName());

					statement.setBoolean(
						5,
						!userUpdate.getLastName().isEmpty()
					);
					statement.setString(6, userUpdate.getLastName());

					statement.setBoolean(7, !userUpdate.getEmail().isEmpty());
					statement.setString(8, userUpdate.getEmail());

					String salt = !userUpdate.getPassword().isEmpty()
						? HashUtils.generateSalt()
						: "";
					String hash = !userUpdate.getPassword().isEmpty()
						? HashUtils.hashWithSalt(userUpdate.getPassword(), salt)
						: "";

					statement.setBoolean(
						9,
						!userUpdate.getPassword().isEmpty()
					);
					...
					if (!userUpdate.getAvatar().isEmpty()) {
					...
					statement.setString(15, userUpdate.getAuthKey());

					int rowsUpdated = statement.executeUpdate();
					return rowsUpdated > 0
						? UserEnums.USER_UPDATED
						: UserEnums.USER_UPDATE_FAILED;
				} catch (SQLException | NoSuchAlgorithmException e) {
					throw new RuntimeException(e);
				}
			}
	...
	public boolean insertUser(User userToInsert) {
		...
		CompletableFuture<Boolean> insertUserCompletableFuture = CompletableFuture.supplyAsync(() -> {
				try (
					...
					String salt = HashUtils.generateSalt();
					String hash = HashUtils.hashWithSalt(
						userToInsert.getPassword(),
						salt
					);
					String authKey = HashUtils.hashWithSalt(
						userToInsert.getEmail(),
						salt
					);
					ByteArrayInputStream inputStream = new ByteArrayInputStream(
						Base64.getDecoder().decode(userToInsert.getAvatar())
					);
	...
	public boolean updateUserOwnedStoreUUID(
		String ownedStoreUUID,
		String ownUUID,
		boolean isDeleteSoreUUID
	) {
		...
		CompletableFuture<Boolean> updateUserOwnedStoreUUIDCompletableFuture = CompletableFuture.supplyAsync(() -> {
				try (Connection conn = DatabaseVerification.getConnection();) {
					PreparedStatement statement;

					if (isDeleteSoreUUID) {
						statement =
							conn.prepareStatement(
								"UPDATE users SET ownedStoreUUID = '' WHERE uuid = '" +
								ownUUID +
								"'"
							);
					} else {
						statement =
							conn.prepareStatement(
								"UPDATE users SET ownedStoreUUID = '" +
								ownedStoreUUID +
								"' WHERE uuid = '" +
								ownUUID +
								"'"
