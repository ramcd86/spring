  ...
  public UserEnums updateUser(UserUpdate userUpdate) {
    ...
    CompletableFuture<UserEnums> updateUsCompletableFuture = CompletableFuture.supplyAsync(() -> {
        try (
          Connection conn = DatabaseVerification.getConnection();
          PreparedStatement statement = conn.prepareStatement(
            "UPDATE Users SET userName = COALESCE(?, userName), firstName = COALESCE(?, firstName), lastName = COALESCE(?, lastName), email = COALESCE(?, email), hash = COALESCE(?, hash), salt = COALESCE(?, salt), avatar = COALESCE(?, avatar) WHERE authKey = ?"
          )
        ) {
